...
export const setPendingOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    if ((error as FirebaseError).code === "permission-denied") {
      return res
        .status(403)
        .json({ error: "You don’t have permission to update pending orders." });
    }
...
export const readNewPendingOrder = async (req: Request, res: Response) => {
  try {
    ...
    if (!transactionId || !uid) {
      return res
        .status(400)
        .json({ error: "transactionId and uid is required" });
    }
    ...
    await firestoreDb.runTransaction(async (transaction) => {
      ...
      if (!usersThatChecked.includes(uid)) {
        const updatedCheckedBy = Array.from(
          new Set([...usersThatChecked, uid])
        );
        transaction.update(orderRef, { checkedBy: updatedCheckedBy });
      }
    });
    ...
    if ((error as FirebaseError).code === "permission-denied") {
      return res
        .status(403)
        .json({
          error: "You don't have permission to update this pending order.",
        });
    }
    if ((error as FirebaseError).code === "unavailable") {
      return res
