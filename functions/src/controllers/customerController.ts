...
export const addCustomer = async (req: Request, res: Response) => {
  try {
    ...
    const existingCustomerRef = customerRef.where(
      "customerName",
      "==",
      customerBody.customerName
    );
    await firestoreDb.runTransaction(async (transaction) => {
      const customer = await transaction.get(existingCustomerRef);
      if (!customer.empty) {
        const conflict = customer.docs.some(
    ...
    if ((error as Error).message === "CUSTOMER_CONFLICT") {
      return res
        .status(409)
        .json({ error: `${req.body.customerName} already exists` });
    }
...
export const updateCustomer = async (req: Request, res: Response) => {
  try {
    ...
    if ((error as FirebaseError).code === "permission-denied") {
      return res
        .status(403)
        .json({ error: "You don’t have permission to update customers." });
    }
    if ((error as FirebaseError).code === "unavailable") {
      return res
        .status(503)
...
export const deleteCustomer = async (req: Request, res: Response) => {
  try {
    ...
    if ((error as FirebaseError).code === "permission-denied") {
      return res
        .status(403)
        .json({ error: "You don’t have permission to delete customers." });
    }
    if ((error as FirebaseError).code === "unavailable") {
      return res
        .status(503)
