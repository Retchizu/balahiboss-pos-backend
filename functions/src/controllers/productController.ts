...
export const addProduct = async (req: Request, res: Response) => {
  try {
    ...
    const existingProductRef = productRef.where(
      "productName",
      "==",
      productName
    );
    ...
    await firestoreDb.runTransaction(async (transaction) => {
      ...
      if (!product.empty) {
        const conflict = product.docs.some(
    ...
    return res
      .status(201)
      .json({ message: `${productName} added successfully` });
  } catch (error) {
    ...
    if (error instanceof ZodError) {
      return res
        .status(400)
        .json({ error: "Invalid product data. Please check your inputs." });
    }

    if ((error as FirebaseError).code === "storage/unauthorized") {
      return res
        .status(403)
        .json({ error: "You don’t have permission to upload this image." });
    }

    if ((error as FirebaseError).code === "storage/quota-exceeded") {
      return res
        .status(507)
        .json({ error: "Storage limit reached. Please try again later." });
    }
    if ((error as Error).message === "PRODUCT_CONFLICT") {
      return res
        .status(409)
        .json({ error: `${req.body.productName} already exists` });
    }
    console.error("AddProduct Error:", error); // full log for devs
  ...
  try {
    ...
    if ((error as FirebaseError).code === "storage/unauthorized") {
      return res
        .status(403)
        .json({
          error: "You don’t have permission to delete this product image.",
        });
    }
    ...
    if ((error as FirebaseError).code === "storage/object-not-found") {
      return res
        .status(404)
        .json({ error: "Product image not found in storage." });
    }
...
export const addStock = async (req: Request, res: Response) => {
  try {
    ...
    if (parseFloat(additionalStock) <= 0) {
      return res
        .status(400)
        .json({ error: "Additional stock must be a positive number" });
    }
    ...
    const newStock =
      (parseFloat(currentProduct!.stock) || 0) + parseFloat(additionalStock);
    ...
    return res
      .status(200)
      .json({ message: `Stock updated successfully. New stock: ${newStock}` });
  } catch (error) {
    ...
    if (error instanceof ZodError) {
      return res
        .status(400)
        .json({ error: "Invalid data. Please check your inputs." });
    }
    if ((error as FirebaseError).code === "permission-denied") {
      return res
        .status(403)
        .json({ error: "You don’t have permission to update product stock." });
    }
    ...
    if ((error as FirebaseError).code === "resource-exhausted") {
      return res
        .status(507)
        .json({ error: "Database quota exceeded. Please contact support." });
    }
