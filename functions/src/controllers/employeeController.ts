...
export const getEmployeeTimesheets = async (req: Request, res: Response) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch employee timesheets." });
  }
};
...
export const updateEmployeeTimeSheet = async (req: Request, res: Response) => {
  try {
    ...
    const updatedLogin: Date = loginTime
      ? new Date(loginTime)
      : data.loginTime.toDate();
    let updatedLogout: Date | null = logoutTime
      ? new Date(logoutTime)
      : data.logoutTime?.toDate() || null;
    ...
    if (updatedLogout) {
      const cappedLogoutIso = capLogoutAtEndOfLoginDay(
        updatedLogin,
        updatedLogout
      );
    ...
    const durationMs = updatedLogout
      ? updatedLogout.getTime() - updatedLogin.getTime()
      : 0;
