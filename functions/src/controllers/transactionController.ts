  ...
  try {
    ...
    if ((error as FirebaseError).code === "permission-denied") {
      return res.status(403).json({
        error: "You don’t have permission to create this transaction.",
      });
    }
...
export const updateTransaction = async (req: Request, res: Response) => {
  try {
    ...
    if ((error as Error).message.startsWith("INSUFFICIENT_STOCK:")) {
      ...
      return res
        .status(400)
        .json({ error: `Insufficient stock for product ${id}` });
    }
    return res.status(500).json({
...
export const calculateTransactionSummary = async (
  ...
  try {
    ...
    for (let i = 0; i < transactionSnapshot.size; i++) {
      const transactionId = transactionSnapshot.docs[i].id;
      const transaction = {
        id: transactionId,
